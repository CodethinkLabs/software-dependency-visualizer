...
def browser_content(path):
    ...
    return bottle.static_file(path, root='examples/')

@app.route('/src/<path:path>')
def browser_content(path):
    '''Serve the browser content as static files.'''
    return bottle.static_file(path, root='src/')

@app.route('/dest/<path:path>')
def browser_content(path):
    '''Serve the browser content as static files.'''
    return bottle.static_file(path, root='dest/')
...
def node_name(node):
    ...
    if 'name' in node.properties:
        print("Attempting to make a name for node %s [using 'name': %s] "%(repr(node),node.properties['name']))
...
def nodes_type(node_type):
    ...
    for node in nodes:
        ...
        result[node_json['uri']] = node_json
...
class Presentation():
    ...
    def __init__(self, root_node,
                 width_relationships=['sw:requires', 'sw:produces', 'sw:calls'],
                 max_width=2,
...
def graph_present(root_node_identifier):
    ...
    def traverse_widthwise(start_node, config, depth=0):
        ...
        for path in paths: # Also never used - there are never any paths
...
def node_info(node_identifier):
    ...
    def node_link(node):
        ...
        return urllib.parse.urlunsplit([
            scheme, netloc, app.get_url('/examples/') + 'index.html',
            query_string, None])
